...
- **Desktop Profile Overlay:** On desktop, clicking a profile opens it as a full-screen overlay instead of navigating away. The feed stays mounted in the background, preserving video playback and scroll position. Uses browser history state (not URL changes) for back button support. Mobile continues to use full page navigation.
