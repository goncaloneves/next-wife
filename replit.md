...
- **High-Resolution Media (Bot API):** Telegram Bot API integration for fetching full-resolution photos/videos. A scheduler polls for new channel posts using `getUpdates` and stores file IDs. The `pollBotUpdates()` function groups updates by `media_group_id` to handle multi-photo albums, sorting by message ID to match `media_urls` order. The `/api/tg-highres-image?file_id=XXX` endpoint streams high-res images from Bot API. Manual syncs also trigger `pollBotUpdates()` for immediate file ID capture. Note: Only NEW posts (after bot was added as channel admin) have file IDs; existing posts use preview images.
