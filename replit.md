...
- **Auto-Fix File ID Mismatches:** Every 10 minutes, the scheduler automatically: (1) Clears `photo_file_ids` for posts where count doesn't match `media_urls` count (prevents wrong images when photos are deleted), (2) If `backfill_chat_id` is configured, auto-backfills 10 posts per sync that are missing file_ids using `forwardMessage` to extract them.
