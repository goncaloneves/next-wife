...
- **Ping-Pong Video System:** `generatePingpong(url)` is the core function — downloads the source video, runs FFmpeg (`split→reverse→concat`, `libx264 ultrafast`, `-profile:v baseline -level 3.1 -pix_fmt yuv420p -movflags +faststart`), and writes to `/tmp/nextwife-pingpong/<md5hash>.mp4`. Encoding uses H.264 Baseline profile + YUV 4:2:0 for universal browser compatibility (required by Safari/iOS). Cache is capped at 20 files (oldest evicted). If the file already exists, it returns instantly with no work. `prewarmPingpongCache()` queries the DB for the 4 most recent video posts and runs all 4 through `generatePingpong` in parallel via `Promise.all`. The `/api/pingpong-video` endpoint supports HTTP range requests (`206 Partial Content`) required by Safari for video playback.
